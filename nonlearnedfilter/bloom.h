...
template <typename ItemType, bool branchless>
class BloomFilter {
public:
  ...
  explicit BloomFilter(const size_t n, double bits_per_item){
    this->size = 0;
    this->k = getBestK(bits_per_item);
  ...
  Status AddAll(std::vector<Slice *> &keys);
  // Report if the item is inserted, with false positive rate.
  Status Contain(const Slice &key) const;
...
template <typename ItemType, bool branchless>
Status BloomFilter<ItemType, branchless>::Add(
    const Slice &key) {
...
template <typename ItemType, bool branchless>
Status BloomFilter<ItemType, branchless>::AddAll(
...
template <typename ItemType, bool branchless>
Status BloomFilter<ItemType, branchless>::Contain(
