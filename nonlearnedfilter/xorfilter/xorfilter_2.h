...
class XorFilter2 {
 ...
 public:
  explicit XorFilter2(const size_t size, double mul, size_t fingerprint_size_) {
    hasher = new HashFamily();
    this->size = size;
    this->arrayLength = 32 + mul * size;
    this->blockLength = arrayLength / 3;
    fingerprints = new FingerprintStorageType(arrayLength, fingerprint_size_);
    std::cout << "XORfilter Size:" << (arrayLength*fingerprint_size_) / (double)(8*1024*1024)  << std::endl;
  }
  ...
  Status AddAll(const vector<Slice *> keys, const size_t start, const size_t end);

  // Report if the item is inserted, with false positive rate.
  Status Contain(const Slice &item) const;
...
Status XorFilter2<ItemType, FingerprintType, FingerprintStorageType, HashFamily>::AddAll(
    const vector<Slice *> keys, const size_t start, const size_t end) {
    int m = arrayLength;
    ...
    while (true) {
        ...
        for(size_t i = start; i < end; i++) {
            uint64_t hash = XXH3_128bits(&keys[i]->str[0], keys[i]->str.size()).low64;
...
Status XorFilter2<ItemType, FingerprintType, FingerprintStorageType, HashFamily>::Contain(
    ...
    uint64_t hash = XXH3_128bits(&key.str[0], key.str.size()).low64;
